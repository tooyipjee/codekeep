...
authRoutes.post('/register', async (c) => {
  ...
  await createKeep(db, { id: keepId, player_id: playerId, name: `${displayName}'s Keep`, grid_state: gridState, resources });
...
authRoutes.post('/login', async (c) => {
  ...
  const player = await findPlayerByApiKey(db, body.apiKey);
...
authRoutes.post('/github', async (c) => {
  ...
  const player = await findPlayerByGithubId(db, body.githubId);
  if (player) {
    const token = await signJwt(player.id);
  ...
  const gridState = JSON.stringify({ width: 16, height: 16, structures: [] });
  const resources = JSON.stringify(STARTING_RESOURCES);
  await createKeep(db, { id: keepId, player_id: playerId, name: `${body.displayName}'s Keep`, grid_state: gridState, resources });
